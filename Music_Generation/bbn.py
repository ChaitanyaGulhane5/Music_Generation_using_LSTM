...
#    emo = input("Enter emotion(h for happy, s for sad, a for angry): ")
    ...
    waterfalls.write(
        "midi",
        fp=r"C:\Users\Chaitanya\PycharmProjects\project_final\project_final\generated_music\{filename}-{emo}.mid",
    )

def selection():
    root = tk.Tk()
    happyButton = tk.Button(root, text="happy", command=emoHappy, width=30, height=15)
    happyButton.pack(pady=10)
    sadButton = tk.Button(root, text="sad", command=emoSad, width=30, height=15)
    sadButton.pack(pady=10)
    angryButton = tk.Button(root, text="angry", command=emoAngry, width=30, height=15)
    angryButton.pack(pady=10)
    tk.mainloop()
